...
import { promotionsApi } from "../../store/PromotionsSlices"
import CardComponent from "../../component/CardComponent";
const PromotionsScreen = ({ navigation, threshold = 100 }) => {
    const dispatch = useDispatch();
    const onEndReachedCalledDuringMomentum = useRef(true);
    const [currantPage, setCurrantPage] = useState(1);
    const [refreshing, setRefreshing] = useState(false);
    const PROMOTIONCLIST = useSelector(state => state.promotion.promotions);
    ...
    _renderItem = ({ item }) => (
        <View style={{ flex: 1, width: "100%" }}>
            <CardComponent item={item} navigation={navigation} button={true} details={'PromotionsDetails'} />
        </View>
    );
    const renderLoder = () => {
        return (
    ...
    return (
        <SafeAreaView style={styles.container}>
            <AppbarHeader title="Promotions" back={true} home={true} navigation={navigation} />
            <FlatList
                ref={flatListRef}
